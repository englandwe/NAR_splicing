...
#Usage: sjshapePC.py icSHAPE_file GTF_file threads output_file
...
def genToTx(txname,gtfdict):
    ...
    for exon in exonlist:
        ...
        tx_start=tx_stop+1
    return rangelist,typelist,chrlist
...
def shapeByRegion(shapefile,chunkStart,chunkSize):
    ...
    with open(shapefile) as infile:
        ...
        for line in lines:
             ...
             ranges,types,chrs=genToTx(txname,ggtfdict)
             #these are the exon ranges, in transcript positions, and their strandedness
             if types[0] == 'protein_coding':
                 for i in range(0,len(ranges)-1):
                     shapesub=[]
                     if ranges[i][3]-ranges[i][2] > 70:
                         subrange=range(ranges[i][3]-69,ranges[i][3]+1)
                         for pos in subrange:
